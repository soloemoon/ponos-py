...
@pl.api.register_dataframe_namespace('tidydf')
class TidyDF:
    ...
    def filter_df(
        ...
        *,
        ...
        if filter_column is not None and filter_values is not None:
            df = self._df.filter(pl.col(filter_column).is_in(filter_values))
    ...
    def pivot_wider(
        ...
        *,
        ...
        if index is None:
            ...
            while temp_index_col in self._df.columns: 
                temp_index_col = f"_{temp_index_col}"
            df_pivot = self._df.with_columns(pl.lit(1).alias(temp_index_col))
    ...
    def pivot_longer(
        ...
        *,
        ...
        if cols is not None:
            ...
            missing = [c for c in cols_list if c not in self._df.columns]
            if missing:
                raise ValueError(f"Columns not found in DataFrame: {missing}")
            
            # ID columns are everything not being pivoted
            id_vars = [c for c in self._df.columns if c not in cols_list]
            ...
            missing = [c for c in exclude_list if c not in self._df.columns]
            if missing:
                raise ValueError(f"Columns to exclude not found in DataFrame: {missing}")
            
            id_vars = exclude_list
            value_vars = [c for c in self._df.columns if c not in exclude_list]
            
        else:
            # No specification: pivot all columns (no ID vars)
            id_vars = []
            value_vars = list(self._df.columns)
    ...
    def separate_column(
        ...
        *,
        ...
        col_index = self._df.columns.index(column)
        ...
        result = self._df.with_columns(new_col_exprs)
    ...
    def collapse_rows(
        ...
        * "concat": Concatenate string values with custom separator (see concat_sep)
        ...
        missing = [c for c in group_cols if c not in self._df.columns]
        if missing:
        ...
        agg_candidates = [c for c in self._df.columns if c not in group_cols]
        ...
        result = self._df.group_by(group_cols, maintain_order=True).agg(agg_exprs)
    ...
    def expand_list_column(
        ...
        * Longer lists are truncated to the number of columns specified
        ...
        if self._df[column].dtype.base_type() != pl.List:
            raise ValueError(
                f"Column '{column}' must contain lists. "
                f"Found type: {self._df[column].dtype}"
            )
        ...
        if mode == "rows":
            ...
            max_len = self._df.select(pl.col(column).list.len().max()).item()
            
            if max_len is None or max_len == 0:
                # All lists are empty or null
                return self._df.drop(column)
            ...
            result = self._df.with_columns(new_col_exprs).drop(column)
    ...
    def map_column(
        ...
        * It becomes the default value if default is specified
        ...
        # Apply the mapping
        result = self._df.with_columns(mapped_expr)
